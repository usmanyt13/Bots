...
const TOKEN = "NjIyNzEzMzg0NjM4NjExNDU3.XbrWzA.8a9aeC_dtwyzBEOuD93Su29q54o";
...
client.on('message', message => {
    ...
    if (message.content === 'Fine') {
      message.channel.send(`I Am Also Fine.
...
.setFooter(member.displayName, member.user.displayAvatarURL);
  ...
  member.send(`Hi Welcome To The Server`)
    channel.send(wembed);
}); 
...
client.on("guildMemberRemove", member => {
  const byebyechannel = member.guilds.channels.find(ch => ch.name === 'welcome')
member.send(`Bye Bye Hope You Come Again`)
byebyechannel.send();
...
client.on("guildDelete", guild => {
  const botowner = client.users.get("494899216280322049")
